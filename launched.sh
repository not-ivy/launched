...
if [ "$(ls -A instances)" ]; then
  ...
  select version in $(jq -r '.versions[] | .id' <<<"$(cat ./data/version_manifest.json)"); do
    ...
    jq -r '.libraries[] | .downloads | .artifact | .path' "./data/$version/version.json" <<<cat | sed -r 's/null//' | while read -r downloadpath; do
      if [ -z "$downloadpath" ]; then
        continue
      fi
      ./scripts/logs/info.sh "Downloading $downloadpath..."
      mkdir -p "./instances/$name/libraries/$(dirname "$downloadpath")"
      curl --progress-bar "https://libraries.minecraft.net/$downloadpath" -o "./instances/$name/libraries/$downloadpath"
    done
    jq -r ".libraries[] | .downloads | .classifiers | .[\"$system\"] | .path" "./data/$version/version.json" <<<cat | sed -r 's/null//' | while read -r downloadpath; do
      if [ -z "$downloadpath" ]; then
        continue
      fi
      ./scripts/logs/info.sh "Downloading $downloadpath..."
...
unset tempclasspath
jq -r ".libraries[] | .downloads | .classifiers | .[\"$system\"] | .path" "./data/$version/version.json" <<<cat | sed -r 's/null//' | while read -r tempclasspath; do
  if [ -z "$tempclasspath" ]; then
    continue
  fi
  classpath+=("$(pwd)/instances/$name/libraries/$tempclasspath")
...
java -Djava.library.path="$(pwd)/libraries" -cp "$classpathstring:$(pwd)/client.jar" "$(jq -r '.mainClass' "../../data/$version/version.json" <<<cat)"
# Exception in thread "main" java.lang.UnsatisfiedLinkError: no lwjgl in java.library.path: /Users/sourtaste000/Developer/launched/instances/1point8/libraries/org/lwjgl/lwjgl/lwjgl/2.9.1
